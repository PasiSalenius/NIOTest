...
let serverConfig = TLSConfiguration.makeServerConfiguration(certificateChain: [sslCertificate], privateKey: sslPrivateKey)
...
let bootstrap = ServerBootstrap(group: group)
    ...
    .childChannelInitializer { channel in
        channel.pipeline.addHandler(ByteToMessageHandler(HTTPRequestDecoder(leftOverBytesStrategy: .forwardBytes)))
            ...
            .flatMap { channel.pipeline.addHandler(ProblematicHandler()) }
            .flatMap { channel.pipeline.addHandler(HelloHandler()) }
    }
